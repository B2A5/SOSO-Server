...
class VoteboardLikeInfoIntegrationTest {
    ...
    void getVotePostDetail_WithLikeInfo_NoLikes() throws Exception {
        // given - 투표 게시글 생성
        Long votePostId = createVotePost(testUserDetails, "좋아요 정보 테스트");
    ...
    void getVotePostDetail_WithLikeInfo_WithLikes() throws Exception {
        // given - 투표 게시글 생성 및 좋아요 추가
        Long votePostId = createVotePost(testUserDetails, "좋아요 정보 테스트2");

        // 다른 유저가 좋아요
        mockMvc.perform(post("/community/votesboard/" + votePostId + "/like")
    ...
    void getVotePostList_WithLikeInfo() throws Exception {
        // given - 여러 투표 게시글 생성
        Long votePostId1 = createVotePost(testUserDetails, "투표1");
        Long votePostId2 = createVotePost(testUserDetails, "투표2");
        Long votePostId3 = createVotePost(testUserDetails, "투표3");

        // 첫 번째 게시글에 좋아요 2개
        mockMvc.perform(post("/community/votesboard/" + votePostId1 + "/like")
                .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails)));
        mockMvc.perform(post("/community/votesboard/" + votePostId1 + "/like")
                .with(SecurityMockMvcRequestPostProcessors.user(anotherUserDetails)));

        // 두 번째 게시글에 좋아요 1개 (현재 유저만)
        mockMvc.perform(post("/community/votesboard/" + votePostId2 + "/like")
                .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails)));
    ...
    void toggleLike_AndCheckDetailResponse() throws Exception {
        // given - 투표 게시글 생성
        Long votePostId = createVotePost(testUserDetails, "토글 테스트");
