...
import com.example.soso.community.pollboard.domain.dto.VoteRequest;
...
class PollVotingIntegrationTest {
    ...
    void changeVote_Success() throws Exception {
        ...
        mockMvc.perform(put("/community/polls/" + voteboardId + "/vote")
                        .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void cancelVote_Success() throws Exception {
        ...
        // when - 투표 취소
        mockMvc.perform(delete("/community/polls/" + voteboardId + "/vote")
                        .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails)))
                .andExpect(status().isOk());
