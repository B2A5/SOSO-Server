...
import com.example.soso.community.pollboard.comment.domain.dto.PollCommentCreateRequest;
...
class PollCommentIntegrationTest {
    ...
    void createComment_Success() throws Exception {
        ...
        PollCommentCreateRequest request = PollCommentCreateRequest.builder()
                .content("첫 번째 댓글입니다")
                .build();

        // when & then
        mockMvc.perform(post("/community/polls/" + votesboardId + "/comments")
    ...
    void getComments_Empty() throws Exception {
        ...
        // when & then
        mockMvc.perform(get("/community/polls/" + votesboardId + "/comments")
                        .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails)))
    ...
    void updateComment_AsAuthor_Success() throws Exception {
        ...
        PollCommentCreateRequest updateRequest = PollCommentCreateRequest.builder()
                .content("수정된 댓글 내용")
                .build();

        // when & then
        mockMvc.perform(patch("/community/polls/" + votesboardId + "/comments/" + commentId)
                        .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails))
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteComment_AsAuthor_Success() throws Exception {
        ...
        // when - 댓글 삭제
        mockMvc.perform(delete("/community/polls/" + votesboardId + "/comments/" + commentId)
                        .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails)))
    ...
    void createReply_Success() throws Exception {
        ...
        PollCommentCreateRequest replyRequest = PollCommentCreateRequest.builder()
                .content("대댓글입니다")
    ...
    void toggleCommentLike_Success() throws Exception {
        ...
        // when - 좋아요 추가
        mockMvc.perform(post("/community/polls/" + votesboardId + "/comments/" + commentId + "/like")
                        .with(SecurityMockMvcRequestPostProcessors.user(testUserDetails)))
                .andExpect(status().isOk())
    ...
    private Long createVotesboard(CustomUserDetails userDetails, String title) throws Exception {
        ...
        MvcResult result = mockMvc.perform(multipart("/community/polls")
                        ...
                        .param("options[0].content", "옵션 A")
                        .param("options[1].content", "옵션 B")
                        .param("closedAt", endTime.toString())
    ...
    private Long createComment(Long votesboardId, CustomUserDetails userDetails, String content) throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/community/polls/" + votesboardId + "/comments")
                        .with(SecurityMockMvcRequestPostProcessors.user(userDetails))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
