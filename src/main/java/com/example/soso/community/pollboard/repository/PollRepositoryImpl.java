...
import com.example.soso.community.pollboard.domain.entity.QPoll;
...
public class PollRepositoryImpl implements PollRepositoryCustom {
    ...
    @Override
    public List<Poll> findAllBySortAndCursor(PollStatus status, PollSortType sort, Long cursor, int size) {
        QPoll poll = QPoll.poll;
        LocalDateTime now = LocalDateTime.now();

        BooleanBuilder condition = new BooleanBuilder();
        // 삭제되지 않은 게시글만 조회
        condition.and(poll.deleted.eq(false));
        ...
        // 커서 조건 추가
        if (cursor != null) {
            condition.and(getCursorCondition(poll, sort, cursor));
        }

        // 정렬 설정
        OrderSpecifier<?>[] orderSpecifiers = getOrderSpecifiers(poll, sort);
     ...
     * 정렬 기준에 따른 커서 조건 생성
     */
    private BooleanExpression getCursorCondition(QPoll poll, PollSortType sort, Long cursorId) {
        // 커서 ID에 해당하는 게시글의 정렬 기준 값을 가져와서 비교
        // 간단하게 ID만으로 커서 처리 (복잡한 정렬의 경우 서브쿼리 필요)
        return poll.id.lt(cursorId);
    }
