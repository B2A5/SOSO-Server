...
import com.example.soso.community.pollboard.service.PollLikeService;
 ...
 * 투표 게시글 좋아요 API 컨트롤러
 */
@Tag(name = "Poll Like", description = "투표 게시글 좋아요 API")
@Slf4j
@RestController
@RequestMapping("/community/polls/{pollId}/like")
...
public class PollLikeController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "투표 게시글을 찾을 수 없음",
                    content = @Content(
                            schema = @Schema(implementation = ErrorResponse.class),
                            examples = @ExampleObject(value = "{\"code\": \"NOT_FOUND\", \"message\": \"해당 투표 게시글을 찾을 수 없습니다.\"}")
                    )
            )
    })
    @PostMapping
