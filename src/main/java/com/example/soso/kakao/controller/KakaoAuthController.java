...
public class KakaoAuthController {
    ...
    public ResponseEntity<KakaoLoginResult> kakaoLogin(
            @RequestBody KakaoLoginRequest request, HttpSession session, HttpServletResponse response
    ) {
        KakaoLoginResult result = kakaoService.login(request.code(), request.codeVerifier(), session, response);
